...
public class EliteReplicatorRenderer implements BlockEntityRenderer<EliteReplicatorBlockEntity> {
    ...
    @Override
    public void render(EliteReplicatorBlockEntity entity, float partialTicks, PoseStack poseStack, MultiBufferSource multiBufferSource, int combinedLightIn, int combinedOverlayIn) {
        ...
        poseStack.translate(0 , -EliteReplicatorBlockEntity.LOWER_PROGRESS,0);

        var progress = (entity.getProgress() + partialTicks / 100f) / (float) entity.getMaxProgress();
        //progress = 0;

        poseStack.translate(0, EliteReplicatorBlockEntity.LOWER_PROGRESS * progress - 0.001f, 0);
